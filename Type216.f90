...
!Do All of the "Very First Call of the Simulation Manipulations" Here
      If(getIsFirstCallofSimulation()) Then
		...
		Call SetNumberStoredVariables(0,11)                   !The number of static variables that the model wants stored in the global storage array and the number of dynamic variables that the model wants stored in the global storage array
...
!Do All of the First Timestep Manipulations Here - There Are No Iterations at the Intial Time
   ...
   !Set the Initial Values of the Outputs (#,Value)
      ...
      CALL setOutputValue(3, 0.d0)   !Wf
      CALL setOutputValue(4, 0.d0)   !Wm
...
!Get some derived values
	...
	Cum_PPFD = Cum_PPFD + Q_SOL_IN * 2.3d0
	Cum_Tday = Cum_Tday + T_GH
	Cum_Tdaytime = Cum_ndaytime + GT(Q_SOL_IN,0) * T_GH
	Cum_ndaytime = Cum_ndaytime + GT(Q_SOL_IN,0)
	fN_h = fN(T_GH)
	Cum_dNdt = Cum_dNdt + dNdT(fN_h, Timestep)
...
!Set the Outputs from this Model (#,Value)
      CALL setOutputValue(1, LAI) !Leaf area index [m2/m2]
      ...
      CALL setOutpuValue(7, Wm * AREA_GH * VEG_FRAC)  ! Total Weight of mature fruit [g]
...
!Function to calculate the node derivative (dNdt) 	
      DOUBLE PRECISION FUNCTION dNdt(fN_h, Timestep)
	    DOUBLE PRECISION Nm, fN_h, Timestep
		Nm = (0.5/24) * Timestep ! Nm value per timestep
		dNdt = Nm * fN_h
	RETURN
      END
...
! Function to calculate the derivative of leaf area index (dLAIdt)
      DOUBLE PRECISION FUNCTION dLAIdt(LAI, LAImax, dens, N, lambda_d, dNdt_d)
	    ...
	    ELSE
    		a = exp(beta * (N - Nb))
    		dLAIdt = dens * sigma * lambda_d * a * dNdt_d / (1.d0 + a)
            ENDIF
	RETURN
      END

!Function to calculate the above ground biomass accumulation (dWdt)
      DOUBLE PRECISION FUNCTION dWdt(LAI,LAImax, dWfdt_d, GRnet_d, dens, dNdt_d)
	DOUBLE PRECISION  LAI, dWfdt_d, GRnet_d, dens, dNdt_d, LAImax, p1, Vmax, dens, dNdt_d, a, b, GRnet_d  
...
!Maximum leaf photosynthetic rate; Jones(1991)
	DOUBLE PRECISION LFmax(C_CO2)
		DOUBLE PRECISION tau, C_CO2
...
! function for calculating photosynthesis rate
	DOUBLE PRECISION Pg(LFmax_d, PGRED_d, PPFD_d, LAI)
...
! function for calculating daily integral of maintenance respiration
	DOUBLE PRECISION Rm(T, W, Wm)
		DOUBLE PRECISION T, W, Wm, Q10, rm
		Q10 = 1.40d0 ! P Jones(1991)
		rm = 0.016d0 ! P Jones(1999)
		Rm = (Q10 ** ((T-20.0d0)/10.0d0)) * rm * (W - Wm)
	 RETURN
	END
...
! FUNCTION TO MODIFY PARTITIONING TO FRUIT VS. AVERAGE DAILY TEMPERATURE
	DOUBLE PRECISION FUNCTION fF(Td)
    		DOUBLE PRECISION Td
...
! function to calculate fruit dry weight increase rate
	DOUBLE PRECISION FUNCTION dWfdt(GRnet_d, N, g_d) ! THIS FUNCTION HAS BEEN CORRECTED
    		DOUBLE PRECISION N, NFF, alpha_F, v, fF_d
    		...
    		IF (N <= NFF) then
        		dWfdt = 0.0d0
    		ELSE
        		dWfdt = GRnet_d * alpha_F * fF_d * (1.0d0 - exp(v * (NFF - N))) * g_d
    		ENDIF
	 RETURN
	END